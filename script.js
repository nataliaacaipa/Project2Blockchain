		...
		"outputs": [
			{
				...
				"internalType": "struct Splitwise.IOweYou[]",
				"name": "",
				"type": "tuple[]"
			}
		],
...
var contractAddress = '0x793A1452327d566e9cb201BB85C092EF8FF1D954'; // FIXME: fill this in with your contract's address/hash
...
async function getTotalOwed(user) {
	...
	amountC = 0;
	for(var i=0; i<listIOUs.length; i++){
		if (user == listIOUs[i].debtor){
			amountD += parseInt(listIOUs[i].amount);
		}
		if (user == listIOUs[i].creditor){
			amountC += parseInt(listIOUs[i].amount);
		}

	}
	return amountD-amountC;
...
async function getLastActive(user) {
	...
	for(var i=0; i<listIOUs.length; i++){
		if (user == listIOUs[i].debtor || user == listIOUs[i].creditor){
			if(date<listIOUs[i].date){
				date = listIOUs[i].date;
			}
		}
